...
var Reversi = function () {
  ...
  _createClass(Reversi, [{
    ...
    value: function returnSymmetriesAsBN() {
      var symmetries = new _bn2.default(0);
      if (this.RotSym) symmetries = symmetries.add(new _bn2.default(parseInt('10000', 2)));
      if (this.Y0Sym) symmetries = symmetries.add(new _bn2.default(parseInt('01000', 2)));
      if (this.X0Sym) symmetries = symmetries.add(new _bn2.default(parseInt('00100', 2)));
      if (this.XYSym) symmetries = symmetries.add(new _bn2.default(parseInt('00010', 2)));
      if (this.XnYSym) symmetries = symmetries.add(new _bn2.default(parseInt('00001', 2)));
    ...
    value: function toSVG() {
      ...
      var id = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.byteBoard;
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (id !== _this6.byteBoard && id !== "0x" + _this6.byteBoard) {
          _this6.byteBoardPopulateBoard(id);
          _this6.calcWinners();
          _this6.isSymmetrical();
        }
        ...
        var svg = '<?xml version="1.0" encoding="UTF-8"?><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="' + size + 'px" height="' + size + 'px" viewBox="-10 -10 ' + (size + 20) + ' ' + (size + 20) + '" enable-background="new 0 0 ' + size + ' ' + size + '" xml:space="preserve" >';

        if (_this6.whiteScore < _this6.blackScore) {
          fill = black;
          stroke = black;
        } else if (_this6.whiteScore > _this6.blackScore) {
          fill = white;
          stroke = white;
        } else {
          fill = grey;
        ...
        for (var i = 0; i < 64; i++) {
          var row = Math.floor(i / 8);
          var col = i % 8;
          switch (_this6.board[row][col]) {
            case _this6.BLACK:
              if (_this6.whiteScore < _this6.blackScore) continue;
              fill = black;
              stroke = 'none';
              break;
            case _this6.WHITE:
          ...
          var x = (row + 1) * (size / 12) + size / 8;
          var y = (col + 1) * (size / 12) + size / 8;
